...
class Discriminator(object):
	...
	def __init__(self, ndf=15, name="d_"):
		with tf.variable_scope(name):
			...
			self.W = {
				'h1': weight_variable('h1', [4, 4, 4, 4, ndf]),
				'h2': weight_variable('h2', [4, 4, 4, ndf, ndf*2]),
				'h3': weight_variable('h3', [4, 4, 4, ndf*2, ndf*4]),
				'h4': weight_variable('h4', [4, 4, 4, ndf*4, ndf*8]),
				'h5': weight_variable('h5', [4, 4, 4, ndf*8, ndf*16]),
				'h6': weight_variable('h6', [4, 4, 4, ndf*16, ndf*32]),
				'md': weight_variable('md', [1*1*1*ndf*32, self.n_kernels*self.dim_per_kernel]),
				'h7': weight_variable('h7', [1*1*1*ndf*32+self.n_kernels, 2])
			}

			self.b = {
				'h1': bias_variable('bias-h1', [ndf]),
				'md': bias_variable('bias-md', [self.n_kernels]),
				'h7': bias_variable('bias-h7', [2])
			}
