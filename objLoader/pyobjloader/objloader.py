...
def MTL(filename):
	...
	for line in open(filename, "r"):
		...
		if values[0] == 'newmtl':
			...
			imname = join(dir_name,mtl['map_Kd'])
			if isfile(imname):
				img = imread(imname)
				if len(img.shape) == 3:
					mtl['image'] = img
			else:
				print 'file doesnt exist', imname
		elif 'map' in values[0]:		# # the texture map
			mtl[values[0]] = values[1]
		else:
...
# faces = [ ([vertex_ids],		0 for none, starting from 1
#			[normal_ids],
#			[tex_ids],
#			 mtl_id,			group <<->> mtl
#			 group_id) 
	...
	def __init__(self, filename, swapyz=False):
		...
		for line in open(filename, "r"):
			...
			if values[0] == 'v':
				...
				self.mtl_path = join(dir_name,values[1])
				self.mtl = MTL(self.mtl_path)
			elif values[0] == 'g':
				group = values[1]
				...
				for v in values[1:]:
					...
					if len(w) >= 3 and len(w[2]) > 0:
						...
						norms.append(0)
				# exit(0)
				self.faces.append((face, norms, texcoords, material, group))
...
if __name__ == '__main__':
	print sys.argv
	if len(sys.argv)!=2:
		print 'usage: python objloader.py obj.obj'
		exit(0)
