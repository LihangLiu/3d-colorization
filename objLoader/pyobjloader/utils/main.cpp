...
// input arguments: filename, grid size
static PyObject* pyLoadObj2Vox(PyObject* self, PyObject* args) {
  
  // arguments
  char* filename;
  int N;
  int C = 4;
  if (!PyArg_ParseTuple(args, "si", &filename, &N)) return NULL;
  // printf("load %s %d\n", filename, N);
  ...
  // convert from obj to vox
  LoadObj2Vox(filename, grid);

  // put grid into pyobject
  PyObject *pyGrid = PyList_New(0);
  for (int i=0; i<N*N*N*C; ++i) 
    PyList_Insert(pyGrid, i, PyFloat_FromDouble(grid.get(i)));
  return pyGrid;
}
...
static PyMethodDef mMethods[] = {
    ...
    {"LoadObj2Vox", pyLoadObj2Vox, METH_VARARGS}, 
    {NULL, NULL, 0, NULL}
