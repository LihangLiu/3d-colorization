...
label_real_d = np.zeros([batch_size, 2], dtype=np.float32)
smooth_factor = .25
label_real[:, 0] = 1
label_fake[:, 1] = 1
label_real_d[:, 1] = 1-smooth_factor
...
loss_D += tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=y, labels=tf.constant(label_real_d)))
