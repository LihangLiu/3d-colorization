	...
	for epoch in xrange(0, myconfig.ITER_MAX):	
		...
		for i in xrange(total_batch): 
			...
			batch_loss_G = sess.run(loss_G, feed_dict={	a:batch_dict['a'], 
														z:batch_dict['z'], 
														train:False})
			batch_loss_a = sess.run(loss_a, feed_dict={	a:batch_dict['a'], 
														z:batch_dict['z'], 
														train:False})
			...
			loss_list['a'].append(batch_loss_a)
			loss_list['D'].append(batch_loss_D)
			print "{0}, {1}, {2:.8f}, {3:.8f}, {4:.8f}".format(epoch, i, batch_loss_G, batch_loss_a, batch_loss_D)

		### print loss ### 
		loss_G_mean = np.mean(loss_list['G'])
		loss_a_mean = np.mean(loss_list['a'])
		loss_D_mean = np.mean(loss_list['D'])
		with open(myconfig.loss_csv, 'a') as f:
			msg = "{0}, {1:.8f}, {2:.8f}, {3:.8f}".format(epoch, loss_G_mean, loss_a_mean, loss_D_mean)
			print >> f, msg
			print msg
