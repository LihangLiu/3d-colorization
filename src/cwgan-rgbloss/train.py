...
loss_G_rgb = G.rgb_loss(rgba_,rgba)
loss_G = -tf.reduce_mean(y_) + loss_G_rgb
...
with tf.Session(config=config) as sess:
    ...
    for epoch in xrange(0, myconfig.ITER_MAX):	
        ...
        for i in xrange(total_batch): 
            ...
            b_loss_G,b_loss_G_rgb,b_loss_D = sess.run([loss_G,loss_G_rgb, loss_D], feed_dict=feed_dict)
            print "%d %d %.6f %.6f %.6f %.6f"%(epoch, i, b_loss_G,b_loss_G-b_loss_G_rgb, b_loss_G_rgb, b_loss_D)
            loss_list['G'].append(b_loss_G)
            loss_list['G_rgb'].append(b_loss_G_rgb)
        ...
        loss_G_rgb_mean = np.mean(loss_list['G_rgb'])
        loss_D_mean = np.mean(loss_list['D'])
        with open(myconfig.loss_csv, 'a') as f:
            msg = "%d %.6f %.6f %.6f %.6f"%(epoch,loss_G_mean,loss_G_mean-loss_G_rgb_mean,loss_G_rgb_mean,loss_D_mean)
            print >> f, msg
            print msg
