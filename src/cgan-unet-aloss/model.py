...
class Generator(object):
	...
	def __init__(self, z_size=5, name="g_"):
		with tf.variable_scope(name):
			...
			self.W = {
				...
				'dh2': weight_variable([4, 4, 4, 16, 32+16]),
				'dh3': weight_variable([4, 4, 4, 4, 16+4])
			}
	...
	def __call__(self, a, z, train):
		...
		dh1 = tf.concat([dh1,h2], -1)	#(n,8,8,8,32+16)
		dh2 = tf.nn.relu(vbn(deconv3d(dh1, self.W['dh2'], [shape[0], 16,16,16,16]), 'g_vbn_2')) #(n,16,16,16,16)
		dh2 = tf.concat([dh2,h1], -1)	#(n,16,16,16,16+4)
		x = tf.nn.tanh(deconv3d(dh2, self.W['dh3'], [shape[0], 32,32,32,4]) + self.b['dh3']) 	#(n,32,32,32,4)

		# alpha loss
		loss_a = self.alpha_loss(a, x)

		return x, loss_a

	def alpha_loss(self, a, rgba):
		# a: (n,32,32,32,1), rgba: (n,32,32,32,4)
		shape = a.get_shape().as_list()		

		a = tf.reshape(a, [-1,1])
		rgb_, a_ = tf.split(rgba, [3,1], 4)
		a_ = tf.reshape(a_, [-1,1])
		n = a.get_shape().as_list()[0]
		loss_a = tf.nn.l2_loss(a-a_)/n
