...
# filter: (D,H,1,C_in,C_out) | (D,1,W,C_in,C_out) | (1,H,W,C_in,C_out)
# 	
...
def flt2_5d_to_3d(filter, axis, depth):
	...
	with tf.variable_scope(ALPHA_NAME): 
		c_list = 10*bias_variable([depth,])
	filter_3d = [filter*c_list[i] for i in xrange(depth)]
	filter_3d = tf.concat(filter_3d, axis)

	print shape_3d
	print c_list.get_shape()
...
def weights_2_5_d(shape):
	...
	filter_3d = []
	c_out = shape[-1]
	for axis in [0,1,2]:		# projected axises, (4,4,1,c_in,c_out/3)
		for co in xrange(c_out/3/5):	# on c_out, (4,4,1,c_in,5)
			new_shape = shape[:]	# must be deep copy here
			new_shape[axis] = 1
			new_shape[-1] = 5
			filter2_5d = weight_variable(new_shape)
			filter_3d.append(flt2_5d_to_3d(filter2_5d, axis, shape[axis]))	# (4,4,4,c_in,5)
