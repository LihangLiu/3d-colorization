...
class Generator(object):
	...
	def __init__(self, z_size=5, ngf=9, name="g_"):
		with tf.variable_scope(name):
			...
			self.W = {
				'hz': weight_variable([z_size, 64*64*64*1]),
				...
				'h6': weight_variable([4, 4, 4, ngf*16, ngf*32]),

				'dh1': weight_variable([4, 4, 4, ngf*16, ngf*32]),
				'dh2': weight_variable([4, 4, 4, ngf*8, ngf*16]),
				'dh3': weight_variable([4, 4, 4, ngf*4, ngf*8]),
				'dh4': weight_variable([4, 4, 4, ngf*2, ngf*4]),
				'dh5': weight_variable([4, 4, 4, ngf, ngf*2]),
				'dh6': weight_variable([4, 4, 4, 3, ngf])
			}
			...
			self.bn6 = BatchNormalization([ngf*32], 'bn6')
	...
	def __call__(self, a, z, train):
		...
		# deconv
		dh1 = tf.nn.relu(vbn(deconv3d(h6, self.W['dh1'], [shape[0],  2,2,2,ngf*16]), 'g_vbn_1')) #(n,2,2,2,f*16)
		dh2 = tf.nn.relu(vbn(deconv3d(dh1, self.W['dh2'], [shape[0], 4,4,4,ngf*8]), 'g_vbn_2')) #(n,4,4,4,f*8)
		dh3 = tf.nn.relu(vbn(deconv3d(dh2, self.W['dh3'], [shape[0], 8,8,8,ngf*4]), 'g_vbn_3')) #(n,8,8,8,f*4)
		dh4 = tf.nn.relu(vbn(deconv3d(dh3, self.W['dh4'], [shape[0], 16,16,16,ngf*2]), 'g_vbn_4')) #(n,16,16,16,f*2)
		dh5 = tf.nn.relu(vbn(deconv3d(dh4, self.W['dh5'], [shape[0], 32,32,32,ngf]), 'g_vbn_5')) #(n,32,32,32,f)
		rgb = tf.nn.tanh(deconv3d(dh5, self.W['dh6'], [shape[0], 64,64,64,3]) + self.b['dh6']) 	#(n,64,64,64,3)
...
class Discriminator(object):
	...
	def __init__(self, ndf=15, name="d_"):
		with tf.variable_scope(name):
			...
			self.W = {
				'h1': weight_variable([4, 4, 4, 4, ndf]),
				'h2': weight_variable([4, 4, 4, ndf, ndf*2]),
				'h3': weight_variable([4, 4, 4, ndf*2, ndf*4]),
				'h4': weight_variable([4, 4, 4, ndf*4, ndf*8]),
				'h5': weight_variable([4, 4, 4, ndf*8, ndf*16]),
				'h6': weight_variable([4, 4, 4, ndf*16, ndf*32]),
				'h7': weight_variable([1*1*1*ndf*32, 2])
			}
