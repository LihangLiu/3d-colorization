...
def flt2_5d_to_3d(filter, axis, depth):
	shape_3d = filter.get_shape().as_list()		# eg, (4,4,2,c_in,c_out), axis=2, depth=4
	#
...
def weights_2_5_d(shape):
	...
	filter_3d = []
	c_out = shape[-1]
	for co in range(c_out):		# iterate c_out
		new_shape = shape[:]	# must be deep copy here
		axis = co%3				# select a projection axis
		new_shape[axis] = 2
		new_shape[-1] = 1		# eg, (4,4,2,c_in,1)
